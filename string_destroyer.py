...
def nand_assigner(nand_index):			#Assigns inputs and output to a NAND gate which was found at nand_index in copy_list
	...
	if((copy_list[nand_index + 2] is in list_of_alphabets) and (copy_list[rand_index + 3] is in list_of_alphabets)):  	#that is both are pure inputs
	...
	elif((copy_list[nand_index + 2] is in list_of_alphabets] and (copy_list[nand_index + 3] == "NAND")):	#first pure input and second is output from NAND
		...
		if(copy_list[second_input_index] is in list_of_alphabets):      #if the second input is a pure input 
...
def string_destroyer(string_temp, input_coordinates, output, forbidden_coordinates ):	#inputs: operation string, coordinates of the inputs
	...
	global copy_list = deepcopy(temp_list)						#deepcopy
	
	global Gate_list = []										#Gate_list will be a list of coordinates of gates i.e. [[Gx1,Gy1],[Gx2,Gy2],...]
	global forbidden_list = forbidden_coordinates				#list of forbidden coordinates
	global static_input_list = input_coordinates				#list of coordinates of inputs like a,b,c,d...
	global gate_input_list = []									#list of coordinates of inputs to gates i.e. [[[g1_inpx1,g1_inpy1],[g1_inpx2,g1_inpy2]],[[..
	global output_list = []										#list of coordinates of outputs of gates [[ox1,oy1],[ox2,oy2]...]
	global output_pin = output									#coordinates of output_pin
	global gate_input_found_list = []							#ith element will denote number of inputs found for the gate i.e 0,1 or 2
	
	global wire_length = 0	
	
	global total_gates = 0 											#total number of gates in the expression
	...
	possible_coordinates = [[x, y] (for x in range(17) for y in range(17)) and [x,y] not in forbidden_list]	
	Gate_list = random.sample(possible_coordinates, total_gates)  	#all the gates are assigned random positions
					
	global current_gate_number = -1	
